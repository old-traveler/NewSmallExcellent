...
public class AuthenticationActivity extends BaseMvpActivity<AuthenticationPresenter>
    ...
    @OnClick({R.id.radiogroup, R.id.authentication_img_photoOne, R.id.authentication_img_photoTwo, R.id.authentication_but})
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == Constans.REQUEST_CODE_CHOOSE && resultCode == RESULT_OK) {
            ...
            if (result != null && result.size() > 0) {
                if (requestCode == Constans.CHOICE_JUDGMENT_ONE){
                    FileUtil.getFilePath(this,result.get(0));
                    ImageRequestHelper.loadImage(this, UCrop.getOutput(data), authenticationImgPhotoOne);
